...
TAB3_SERVER <- function(input, output, session) {
  ...
  txt1 <- c(c1 = "Human machine interface for ABC computer applications",
  ...
  txt2 <- c(d1 = "Shipment of gold damaged in a fire",
...
# The User SVD the text
  shiny::observeEvent( c(input$select_text,
                         input$vectors_text),{
...
# Reconstruct 
  ...
  output$reconstruct_mt_text_tbl <- DT::renderDataTable({
    data_table_fun(df = reconstruct_mt_text, 
...
shiny::wellPanel(
        ...
        column(6,       
               ...
               shiny::sliderInput(inputId = "vectors_text", 
