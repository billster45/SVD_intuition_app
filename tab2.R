...
img <- shiny::reactive({
  ...
  shiny::observeEvent(input$filedata,{
    ...
    output$imgPlot <- shiny::renderPlot({graphics::plot(img(),axes=FALSE, main = "Orginal image")})
...
# Runs based on the image selected
  shiny::observeEvent(input$select_image,{
    ...
    output$imgPlot <- shiny::renderPlot({graphics::plot(image,axes=FALSE,main = "Original image")})
    ...
    #find knee
    knee <- inflection::uik(x = var_expl_df$singular_vectors, 
    ...
    output$var_explained <- shiny::renderPlot({
      shiny::isolate(     var_expl_df %>% 
                            ggplot2::ggplot() +
                            ggplot2::aes(x = singular_vectors,
                                         y = var_expl,
                                         ) +
                            ggplot2::geom_point() +
                            ...
                            ggforce::facet_zoom(xlim = c(1,knee),
                                                horizontal = FALSE,
                                                zoom.size = 0.8) +
                            ggplot2::labs(
...
# vectors change
shiny::observeEvent(c(input$vectors,
                      input$select_image,
                      input$filedata),{
  ...
  shiny::updateSliderInput(session, "vectors", max = ncol(SVD$u))
    ...
    output$reconstruct_mt_tbl <- DT::renderDataTable({
      data_table_fun(df = reconstruct_mt, 
  ...
  shiny::wellPanel(
      ...
      shiny::fluidRow(
                 ...
                 column(6,
                        ...
                        shiny::sliderInput(inputId = "vectors", 
